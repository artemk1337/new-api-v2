...
- This project uses GORM `AutoMigrate` plus explicit migration helpers instead of versioned `up/down` migration files. Any schema change must be safe for rolling deploys and app rollback: the old binary should tolerate the migrated schema, the new binary should tolerate the pre-migration schema until startup migration runs, and destructive changes must be split across releases or guarded by compatibility code. Review both upgrade and downgrade/rollback behavior before shipping database changes.
